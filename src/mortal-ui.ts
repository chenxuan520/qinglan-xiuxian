...
export function townEventContent(save: SaveData, npc: TownResident) {
  ...
  if (npc.id === 'market') {
    ...
    actions = `<p>${job.desc}</p><p class="panel-note">耗时 ${job.years} 年 · ${job.stones ? `${job.stones} 灵石${job.iron ? ` + ${job.iron} 玄铁` : ''}` : '30% 概率获赠 8 灵石'}<br>接下后在城镇内推进，离开暂停，完成后不自动重复。</p><button class="primary-button" data-action="mortal-activity" data-id="${npc.id}" ${save.mortal.activity ? 'disabled' : ''}>${save.mortal.activity ? '已有进行中的委托或研习' : npc.id === 'tea' ? '入座听书' : '接下这件事'}</button>`;
  }
