...
觉醒同轮对王衰减集中在 `Game.hitEnemy()`，只有带 `bossAttack` 的子攻击且目标为非独立天劫的妖王才生效。同次施法共享 `volley.hitBosses`，每个子攻击另存目标倍率；火球直击与爆炸共用该倍率，不修改弹丸基础伤害。快照以 `bossVolleys` 表保存共享关系，弹丸引用索引，读取时校验并重建；无长期注册表，弹丸消失后上下文随引用释放。旧弹丸缺少记录时保留原伤害。`resetLifetime()` 默认直接进入新命盘，仅圆满终章调用显式保留故乡流程。
